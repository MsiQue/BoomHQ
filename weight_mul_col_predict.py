...
from global_info import from_mul_col_query_get_weight
...
def pred(w1_list, w2_list, th_list, labels, large_k_list, split_ratio=(0.7, 0.15, 0.15), epochs=50, batch_size=32,
         learning_rate=0.001):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    inputs = torch.tensor(np.stack([w1_list, w2_list, th_list], axis=1), dtype=torch.float32)
    ...
    model = MultiTaskNet(input_size=3, num_classes_labels=num_classes_labels,
...
def solve(query_sql, file_list, threshold_list):
    ...
    label_dict = {'table_scan' : 0, 'sql_1' : 1, 'sql_2' : 2, 'sql_12' : 3}
    label_list = ['table_scan', 'sql_1', 'sql_2', 'sql_12']
    w1_list = []
    w2_list = []
    th_list = []
    labels = []
    ...
    for i in range(min_id, max_id+1):
        ...
        for threshold in threshold_list:
            ...
            table_scan_t = new_dict[i][100]['table_scan'][1]
    ...
    for i, (w1, w2, th, label, large_k) in enumerate(zip(w1_list, w2_list, th_list, labels, large_k_list)):
        print(f'{i:20} {w1:20} {w2:20} {th:20} {label:20} {large_k:20}')
    ...
    for i in range(min_id, max_id+1):
        w1, w2 = from_mul_col_query_get_weight(query_sql[i][0])
        for threshold in threshold_list:
            _input = torch.tensor([[w1, w2, threshold]]).float().to(device)
...
if __name__ == '__main__':
    query_sql_path = 'query/10000_weight_mul_col_query_sql_[part]_limit100.pickle'
    query_sql = pickle.load(open(query_sql_path, 'rb'))
    threshold_list = [0.8]
    file_root_path = 'result/weight_mul_col'
    file_list = []
    for file_name in os.listdir(file_root_path):
        if file_name.startswith('10000_weight_mul_col_query_result_[part]___['):
