...
from gen_mul_col_query import get_query_vector, extract_nth_distance_query, gen_where_clause
from global_info import get_database_cursor, execute_sql_with_args_explain_returned, convert_to_list, calc, get_range_list, get_number_column_list, calc_selectivity

def hybrid_weight_mul_col_query_topk_sql(column_name, table_name, where_clause, w1, vector_column_1, vector_1, w2, vector_column_2, vector_2, limit, explain = False, opt = '<->'):
    prefix = 'EXPLAIN ' if explain else ''
    return prefix + f"SELECT {column_name} FROM {table_name} {where_clause} ORDER BY {w1} * ({vector_column_1} {opt} '%s') + {w2} * ({vector_column_2} {opt} '%s') LIMIT {limit} ;" % (vector_1, vector_2)

def gen_hybrid_mul_col_query_sql(cursor, table_name, vec_column_name_1, vec_column_name_2, N, limitk, threshold, file_name_suffix = ''):
    ...
    for i in range(N):
        while True:
            where_clause = gen_where_clause(L, ['AND'])
            ...
            w1 = random.random()
            w2 = 1.0 - w1
            sql = hybrid_weight_mul_col_query_topk_sql('id', table_name, where_clause, w1, vec_column_name_1, v1, w2, vec_column_name_2, v2, limitk)
            selectivity = calc_selectivity(cursor, sql)
            if selectivity > threshold:
            # if selectivity > threshold and selectivity < 0.01:
                target_sql = sql
    ...
    filepath = f'query/{N}_hybrid_weight_mul_col_query_sql_[{table_name}]_limit{limitk}{file_name_suffix}.pickle'
...
if __name__ == '__main__':
    ...
    gen_hybrid_mul_col_query_sql(cursor, 'part', 'p_comment_vector', 'p_name_vector', N, 100, 0.0001)
    ...
    # save_path = f'result/weight_mul_col/{N}_hybrid_weight_mul_col_query_result_[{table_name}]_limit100.pickle'
