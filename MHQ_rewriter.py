...
from data_encoder import data_encoder
from query_encoder import neighborhood_probe
...
def pred(r_loss_list, w1_list, w2_list, r1_list, r2_list, selectivity_list, th_list, labels, large_k_list, split_ratio=(0.7, 0.15, 0.15), epochs=50, batch_size=32,
         learning_rate=0.001):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    inputs = torch.tensor(np.stack([r_loss_list, w1_list, w2_list, r1_list, r2_list, selectivity_list, th_list], axis=1), dtype=torch.float32)
    ...
    model = MultiTaskNet(input_size=7, num_classes_labels=num_classes_labels,
...
def from_mul_col_query_get_vector(sql):
    t = sql.split('<->')
    print(t[1], t[2])
    f = lambda x : eval(eval(x.split(')')[0]))
    print(f(t[1]), f(t[2]))
    print(type(f(t[1])), type(f(t[2])))
    return f(t[1]), f(t[2])

def parse_col_ranges(scalar_list, col_ranges):
    # print(col_ranges)
    res = []
    for scalar in scalar_list:
        sca_name = scalar.upper()
        if sca_name in col_ranges:
            t = col_ranges[sca_name][0]
            g = 0 if t is None else t
        else:
            g = 0
        res.append(torch.tensor([[g]]))
    return res

def solve(cursor, table_schema, query_sql, file_list, threshold_list):
    ...
    data_encoder_model = data_encoder(
        n_vector_cols=len(table_schema['vector_column']),
        vector_dims_list=table_schema['vector_columm_dim'],
        n_scalar_cols=len(table_schema['scalar_column']),
        **MODEL_PARAMS
    )
    # data_encoder_model.load_state_dict(torch.load("./model/data_encoder.pth"))
    ...
    for i in range(min_id, max_id+1):
        ...
        v1, v2 = from_mul_col_query_get_vector(query_sql[i][0])
        vec = [torch.tensor(v1).unsqueeze(0), torch.tensor(v2).unsqueeze(0)]
        # print(vec)
        col_ranges = parse_where_clause(get_where_clause_from_sql(query_sql[i][0]))
        sca = parse_col_ranges(table_schema['scalar_column'], col_ranges)
        # print(sca)

        reconstruction_loss = data_encoder_model.get_reconstruction_error(vec, sca).item()
        print(reconstruction_loss)

        r1 = neighborhood_probe(cursor, query_sql[i][0], table_schema['scalar_column'], table_schema['vector_column'][0], v1)
        r2 = neighborhood_probe(cursor, query_sql[i][0], table_schema['scalar_column'], table_schema['vector_column'][1], v2)
        ...
        for threshold in threshold_list:
            ...
            r1_list.append(r1)
            r2_list.append(r2)
            r_loss_list.append(reconstruction_loss)
    ...
    for i, (r_loss, w1, w2, r1, r2, selectivity, th, label, large_k) in enumerate(zip(r_loss_list, w1_list, w2_list, r1_list, r2_list, selectivity_list, th_list, labels, large_k_list)):
        print(f'{i:20} {r_loss:20} {w1:20} {w2:20} {r1:20} {r2:20} {selectivity:20} {th:20} {label:20} {large_k:20}')
    ...
    for i in range(min_id, max_id+1):
        ...
        for threshold in threshold_list:
            _input = torch.tensor([[reconstruction_loss, w1, w2, r1, r2, selectivity, threshold]]).float().to(device)
...
if __name__ == '__main__':
    ...
    threshold_list = [0.8]
    file_root_path = '/data/qiuermu/test_106/result/hybrid_weight_mul_col'
    file_list = []
    ...
    table_schema_dict = {'part' : {'vector_column' : ["p_comment_vector", "p_name_vector"], 'vector_columm_dim' : [768, 768], 'scalar_column' : ["p_size", "p_retailprice"]}}

    solve(cursor, table_schema_dict['part'], query_sql, file_list, threshold_list)